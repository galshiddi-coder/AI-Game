...
public class MiniMaxGameImproved {
    ...
    //count number of  mills for black pieces
    public static int CountBMill(char [] b) {
    ...
    //count number of  mills for white pieces
    public static int CountWMill(char [] b) {
    ...
    //counts the number of mills instead of pieces
    public static int StaticEst (char [] board, int depth){

        int blackMills = CountBMill(board);
        int whiteMills = CountWMill(board);
        int numBlackMoves = numBMove(board);
        int numWhiteMoves = numWMove(board);
        
        if (blackMills < 1) {
            return 10000;
        } 
        else if ( whiteMills < 1){
            return -10000;
        }
            ...
            int estm = (1000*(whiteMills - blackMills)- numBlackMoves);
            return estm;
        }
     }
   ...
   //returns the final board output and writes the board on a file
    public static char [] BoardOutput(ArrayList<char[]> board) {
       ...
       //writes the output file
        try {
            FileWriter write = new FileWriter("board12.txt");
            write.write(output);
            write.close();
        } catch (IOException e) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            while(r.hasNextLine()){
                ...
                System.out.println( MiniMax(b, 3));
