...
public class MiniMaxOpeningBlack {
    ...
    public static char [] BoardOutput(ArrayList<char[]> board) {
        ...
        try {
         FileWriter write = new FileWriter("board8.txt");
         write.write(output);
         write.close();
         } catch (IOException e) {
   ...
   // returns new position for black pieces
    public static List <char[]> GenerateMovesOpening (char pos[]){
        List <char[]> L = GenerateAdd(pos);
        return L;
    }
   
   //looks for an empty cell, then adds black piece to it
   public static List <char []> GenerateAdd (char pos[]){
    ...
    for (int j=0; j< pos.length; j++) {
        ...
        board = pos.clone();
        //if empty, set the location = B
        if(pos[j] == 'x'){
            board[j] = 'B';
            //if black pieces form a mill, then L is list of all board options to w/o a W
            if (closeMill(j, board)){
                L = generateRemove(board, L);
            }
            //if B isn't in a mill, then add the board to the list
            else{
                L.add(board);
            }
        } 
